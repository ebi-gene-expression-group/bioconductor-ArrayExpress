...
The first step consists of importing a dataset from ArrayExpress. We chose E-MEXP-1416. This dataset studies the transcription profiling of melanized domamine neurons isolated from male and female patients with Parkinson disease to investigate gender differences.
<<import, results=hide>>=
...
<<qanorm, results=hide>>=
fac = grep("FactorValue",colnames(pData(AEset)), value=T)
